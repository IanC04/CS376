...
\begin{document}
    ...
    \begin{itemize}
        ...
        compares the binary strong classifiers instead of the entire training set to test each image, its running
        time to predict is much faster than K Nearest Neighbors. Additionally, I cached classifiers after training, so
        that each test wouldn't have to recompute the classifiers, but use the pre-trained ones.
        The ship class is one where KNN does better than Adaboosting, and the cat class is one where Adaboosting does
        better than KNN.
