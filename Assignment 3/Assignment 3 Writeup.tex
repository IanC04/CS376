...
\begin{document}
    ...
    \begin{enumerate}
        ...
        To pick 2D key points, I used the SIFT algorithm, then I used the 20 strongest features.\newline
        Duplicates are due to different orientations of the same feature, which provide stability to the image.\newline
        ...
        \item \textit{The first step is to estimate the matrix $\prod$ = [KR, KT], where K $\in$
            ...
            from the smallest eigenvector computation, ou can eliminate one by enforcing that each $\lambda_i$ has to be
            positive in the following two constraints:}
        \begin{center}
